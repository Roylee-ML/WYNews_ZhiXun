...
#define NC_HEIGHT self.parentViewController.parentViewController.navigationController.navigationBar.frame.size.height
...
- (void)viewDidLoad {
    ...
    if (dataArray) {
        ...
        DataModel * model = (DataModel*)dataArray[0];
        
        //第三方下载图片
        [_imgView sd_setImageWithURL:[NSURL URLWithString:model.imgsrc]];
        _titleLable.text=model.title;
        
        self.oneArray =[@[model] mutableCopy];
    ...
    NSLog(@"height_nc = %.2f,status_height = %.2f",NC_HEIGHT,STATUS_HEIGHT);
...
-(void)loadData{
    //网址对象
    NSString * url=@"http://c.m.163.com/nc/article/list/T1401272877187/0-20.html";
...
-(void)refreshFooterView{
    ...
    NSString *url=[NSString stringWithFormat:@"http://c.m.163.com/nc/article/list/T1401272877187/%@.html",pageCount ];
